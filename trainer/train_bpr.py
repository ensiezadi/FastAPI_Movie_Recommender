...
def main() -> None:
    ...
    model = BayesianPersonalizedRanking(factors=64, regularization=0.01, iterations=50)
    model.fit(user_items.T.tocsr())
    save_bundle(MODEL_DIR / "implicit_bpr.joblib", model, user_items, user_to_index, index_to_movie)
